...
class userC
{
    ...
    function modifierUser($user,$email){
        $sql="UPDATE users SET  Full_name=:name1, 'password'=:password WHERE name=:name";
...
try{        
        ...
        $name1=$user->getName();
        $password=$user->getPassword();
        $datas = array( ':name1'=>$name1,':name'=>$name,':password'=>$password);
        $req->bindValue(':name1',$name1);
        $req->bindValue(':name',$name);
...
class recoverC
{
    ...
    function afficherRecover ($recover){
        echo "email: ".$recover->getEmail()."<br>";
        echo "code: ".$recover->getCode()."<br>";
    
    }
    function afficherRecovers(){
        $sql="SElECT * From recovery";
        $db = config2::getConnexion();
        try{
        $liste=$db->query($sql);
        return $liste;
    ...
    function supprimerRecover($email){
